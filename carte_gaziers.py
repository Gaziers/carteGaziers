...
class gazier :
    def __init__(self, name, latitude, longitude, image) -> None:
        self.name = name
...
gaziers = [
    ...
    gazier('Thibault', 45.9070, 6.1084, 'https://images.emojiterra.com/twitter/v12/512px/1f414.png'),
...
folium.Marker(location=epicenter, popup="Centre des Gaziers", icon=icon).add_to(fmap) #adding it to the map

for gazier in gaziers:
    icon = folium.features.CustomIcon(gazier.image, icon_size=40) # Creating a custom Icon
    folium.Marker(location=[gazier.latitude, gazier.longitude], icon=icon, popup=gazier.name).add_to(fmap) #adding it to the map
